...
const CompanySelector: React.FC<CompanySelectorProps> = ({
  ...
  if (companies.length === 1) {
    return <div className={className}>
        {showLabel && <div className="flex items-center gap-1 mb-1 text-sm font-medium text-white">
            <Building className="h-4 w-4" />
            <span>Current Company</span>
          </div>}
        <div className="flex items-center">
          {!showLabel && <Building className="h-4 w-4 text-slate-400 mr-2" />}
  ...
  return <div className={className}>
      ...
      <div className="flex items-center">
        ...
        <Select value={selectedCompany?.id || ''} onValueChange={handleCompanyChange} disabled={companies.length === 0}>
          <SelectTrigger className="w-full text-white border-slate-700 bg-slate-800 hover:bg-slate-700">
