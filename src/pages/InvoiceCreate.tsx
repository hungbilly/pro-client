...
const InvoiceCreate = () => {
  ...
  logDebug('InvoiceCreate component initialized with params:', {
    clientId,
    jobId,
    invoiceId
  });
    ...
    const fetchContractTemplates = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('contract_templates').select('id, name, content').order('name', {
          ascending: true
        });
        if (error) {
  ...
  const checkDuplicateInvoiceNumber = async (number: string, currentInvoiceId?: string) => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('invoices').select('id, number').eq('number', number);
      if (error) {
  ...
  const getBreadcrumbPaths = () => {
    ...
    paths.push({
      label: invoice ? "Edit Invoice" : "New Invoice",
      path: "#"
    });
    return paths;
  };
      ...
      <div className="container py-8">
        <div className="flex flex-col space-y-2 mb-6">
          ...
          <div className="text-sm text-muted-foreground flex items-center">
            {paths.map((path, index) => <React.Fragment key={path.path}>
                {index > 0 && <span className="mx-1">{'>'}</span>}
                {path.path === '#' ? <span>{path.label}</span> : <span className="hover:underline cursor-pointer" onClick={() => navigate(path.path)}>
                    {path.label}
                  </span>}
              </React.Fragment>)}
          </div>
        ...
        {(job || client) && <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            {job && <Card className="bg-[#9ECAE1] text-white border-[#9ECAE1]">
                <CardContent className="pt-6">
                  ...
                  <div className="text-sm">
                    ...
                    {job.location && <div className="flex items-center gap-2 mt-2">
            ...
            {client && <Card className="bg-blue-500 text-white border-blue-600">
                <CardContent className="pt-6 bg-blue-300">
        ...
        <InvoiceForm invoice={invoice} clientId={clientId || job?.clientId} jobId={jobId} invoiceId={invoiceId} contractTemplates={contractTemplates} checkDuplicateInvoiceNumber={checkDuplicateInvoiceNumber} onInvoiceDeleted={handleInvoiceDeleted} />
      </div>
    </PageTransition>;
};
