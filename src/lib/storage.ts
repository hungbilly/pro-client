...
/**
 * Verifies and decodes an invoice token
 * @param token The JWT token containing invoice data
 * @returns The decoded payload or null if invalid
 */
export const verifyInvoiceToken = (token: string): { 
  invoice: Invoice; 
  client: Client; 
  company: {
    id: string;
  ...
  try {
    // Simple base64 decoding for client-side verification
    // Note: This is not secure cryptographic verification, just basic parsing
